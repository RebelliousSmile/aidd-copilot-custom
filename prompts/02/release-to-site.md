---
description: Translate a GitHub app release into marketing content updates on the site
...
The release tag is provided in the prompt (or fetch the latest if not specified).
...
1. Run the appropriate `gh` command to fetch the release:
   - If tag provided: `gh release view <tag> --repo RebelliousSmile/email2markdown.app`
   - If no tag: `gh release list --repo RebelliousSmile/email2markdown.app --limit 1` then fetch it
